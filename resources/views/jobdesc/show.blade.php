...
@section('content')
<div class="container">
    <div class="app-page-header"><div><span class="app-page-eyebrow">Uraian Jabatan</span><h4 class="app-page-title">{{ $jobdesc->jabatan }}</h4><p class="app-page-subtitle">{{ $jobdesc->job_code ?: 'Job code belum ditentukan' }}</p></div><div class="app-page-actions d-flex gap-2"><a href="{{ route('jobdesc.print', $jobdesc) }}" class="btn btn-outline-primary">Cetak / Simpan PDF</a>@if($canManage)<a href="{{ route('jobdesc.edit', $jobdesc) }}" class="btn btn-primary">Ubah</a>@endif<a href="{{ route('jobdesc.index') }}" class="btn btn-outline-secondary">Kembali</a></div></div>
